...
class ClientTest(unittest.TestCase):
  ...
  def test_getDataPoint_identicalBidAskPrice(self):
    quotes = [
      ...
      {'top_ask': {'price': 123.45, 'size': 4}, 'timestamp': '2019-02-11 22:06:30.572453', 'top_bid': {'price': 123.45, 'size': 81}, 'id': '0.109974697771', 'stock': 'DEF'}
    ]
    for quote in quotes:
      stock, bid_price, ask_price, price = getDataPoint(quote)
      self.assertEqual(price, (quote['top_bid']['price'] + quote['top_ask']['price']) / 2)
  ...
  def test_getDataPoint_negativePrices(self):
    quotes = [
      {'top_ask': {'price': -120.0, 'size': 36}, 'timestamp': '2019-02-11 22:06:30.572453', 'top_bid': {'price': -121.0, 'size': 109}, 'id': '0.109974697771', 'stock': 'ABC'},
      {'top_ask': {'price': -123.45, 'size': 4}, 'timestamp': '2019-02-11 22:06:30.572453', 'top_bid': {'price': -123.0, 'size': 81}, 'id': '0.109974697771', 'stock': 'DEF'}
    ]
    for quote in quotes:
      stock, bid_price, ask_price, price = getDataPoint(quote)
      self.assertEqual(price, (quote['top_bid']['price'] + quote['top_ask']['price']) / 2)
